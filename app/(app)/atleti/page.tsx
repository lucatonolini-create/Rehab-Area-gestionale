    ...
    Array.from(weekMap.entries()).forEach(([wk, wkProgs]) => {
      ...
      for (const prog of wkProgs) {
        ...
        const testLines = (prog.tests ?? []).map((t) => {
          ...
          const val = [t.risultato, t.risultatoSx ? `Sx ${t.risultatoSx}` : "", t.risultatoDx ? `Dx ${t.risultatoDx}` : "", t.tempo ? `Tempo: ${t.tempo}s` : "", t.livello ? `Liv: ${t.livello}` : "", t.vo2max ? `Vo2Max: ${t.vo2max}` : "", t.vam ? `VAM: ${t.vam}` : "", t.ginocchioDxSx ? `Dx→Sx: ${t.ginocchioDxSx}` : "", t.ginocchioSxDx ? `Sx→Dx: ${t.ginocchioSxDx}` : "", t.diffDxSx ? `ΔDx→Sx: ${t.diffDxSx}` : "", t.diffSxDx ? `ΔSx→Dx: ${t.diffSxDx}` : ""].filter(Boolean).join(" / ");
          const extras: string[] = [];
